...
const TopicDetailPage = ({ topicId }: { topicId: string }) => {
  ...
  // Scroll to top and build initial summary when item loads
  useEffect(() => {
    if (!item) return;
    window.scrollTo(0, 0);
  ...
  const otherQuestions = allItems.filter(
    (fi) =>
      fi.id !== item.id &&
      ...
      .catch(() => {
        setChatMessages((prev) => [
          ...prev,
          {
            id: `a-${Date.now()}`,
            role: "assistant",
            text: "Sorry, I was unable to retrieve an answer at this time.",
          },
        ]);
        setChatThinking(false);
      });
      ...
      <div className="topic-detail-layout">
        ...
        <div className="topic-chat-panel">
          ...
          <div className="chat-input-area">
            ...
            />
            <button className="chat-send-btn" onClick={handleChatSend} disabled={chatThinking}>
              Send
            </button>
          </div>
        </div>
